...
class TestViews(TestSetUp):
    ...
    def test_get_apartmentByID(self):
        ...
        get_apartments = Apartment.objects.get(apartment_id = response.data['apartment_id'])
        # delete_apartment.destroy()
        # the base url is concatenated with the user ID from the post function
        get_apartment = self.client.get((self.getApartment_url + str(response.data['apartment_id'])))
        update_apartment = self.client.put((self.getApartment_url + str(response.data['apartment_id'])), self.apartment_update)
        apartment_updated = Apartment.objects.get(apartment_id = response.data['apartment_id'])

        self.assertEqual(get_apartment.status_code, 200)
        self.assertEqual(Apartment.objects.count(), 1)
        self.assertNotEqual(get_apartment.data['agent'], 'wreco')
        # pdb.set_trace()
        self.assertEqual(response.data['apartment_id'], get_apartment.data['apartment_id'])
        ...
        delete_apartment = self.client.delete((self.getApartment_url + str(response.data['apartment_id'])))
